...
The `mutation` root operation type is optional; if it is null, that means the service does not support mutations. If provided, it must be an Object type.

Similarly, the `subscription` type is optional; if it is null, the service does
not support subscriptions. If it is provided, it must be an Object type.
...
Note: The GraphQL Object type is inappropriate for re-use here, because
object types can contain fields with arguments, that express circular
references, or references interfaces or unions, none of which are appropriate
